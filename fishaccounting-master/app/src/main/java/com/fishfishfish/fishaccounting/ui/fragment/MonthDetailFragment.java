...
import com.fishfishfish.fishaccounting.ui.activity.AboutActivity;
...
public class MonthDetailFragment extends BaseFragment implements MonthDetailView {
    ...
    @BindView(R.id.float_b1)
    FloatingActionButton float_b1;
    @BindView(R.id.float_b2)
    FloatingActionButton float_b2;
    int part, index;
     ...
     * @param view
     */
    @OnClick({R.id.float_b1,R.id.float_b2, R.id.layout_data, R.id.top_ll_out})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.float_b1:  //拍照
                Intent intent1 = new Intent(getContext(), AboutActivity.class);
                startActivityForResult(intent1, 0);
                break;
            case R.id.float_b2:  //添加
                if (BmobUser.getCurrentUser() == null)
                    SnackbarUtils.show(mContext, "请先登录");
                else {
                    Intent intent2 = new Intent(getContext(), BillAddActivity.class);
